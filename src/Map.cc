...
//add by haotians here for MAP load and save functions of ORB-slam system
...
KeyFrame* Map::ReadKeyFrame(ifstream &f, ORBVocabulary* voc, std::vector<MapPoint*> amp, ORBextractor* orb_ext)
{
    ...
    f.read((char*)&TransformationMatrix.at<float>(0, 3), sizeof(float));
    f.read((char*)&TransformationMatrix.at<float>(1, 3), sizeof(float));
    f.read((char*)&TransformationMatrix.at<float>(2, 3), sizeof(float));
    TransformationMatrix.at<float>(3,3) = 1.;

    cv::Mat Quaternion(1,4, CV_32F);
    f.read((char*)&Quaternion.at<float>(0, 0), sizeof(float));
    f.read((char*)&Quaternion.at<float>(0, 1), sizeof(float));
    f.read((char*)&Quaternion.at<float>(0, 2), sizeof(float));
    f.read((char*)&Quaternion.at<float>(0, 3), sizeof(float));

    //assign rotation matrix to tranformation matrix
    Converter::GetRotationMatFromQuaternion(TransformationMatrix, Quaternion);

    fr.SetPose(TransformationMatrix);
    // near-by keypoints
    ...
    for (int i=0; i<fr.N; i++)
    {
        ...
        f.read((char*)&fDepthValue,    sizeof(float));           // float
    ...
    fr.mvDepth = KeypointDepth;//vector<float>(fr.N,-1);
 ...
 MapPoint* Map::ReadMapPoint(ifstream &f)
{
    ...
    //get point
    cv::Mat PointCoordinate(3,1, CV_32F);
    f.read((char*)&PointCoordinate.at<float>(0), sizeof(float));
    f.read((char*)&PointCoordinate.at<float>(1), sizeof(float));
    f.read((char*)&PointCoordinate.at<float>(2), sizeof(float));

    long int mnFirstKFid=0, mnFirstFrame=0;
    //trasnfer to map points
    MapPoint* mp = new MapPoint(PointCoordinate, mnFirstKFid, mnFirstFrame, this);
    mp->mnId = id;
    return mp;
}
...
bool Map::Load(const string &filename, ORBVocabulary* voc)
{
    ...
    {
        printf("Map: camera is not initialized\n");
        return false;
    }
    ...
    //read all map points
    cerr << "reading " << nb_mappoints << " map points" << endl;
    ...
    printf("map point init done\n");


    //simple handle goes here
    ORB_SLAM2::MapPoint::nNextId = max_id+1;
    //load all these shit map points
    ...
    printf("Spanning tree init done\n");
...
void Map::WriteKeyFrame(ofstream &f, KeyFrame* kf, map<MapPoint*, unsigned long int>& idx_of_mp)
{
    ...
    for (int i=0; i<kf->N; i++)
    {
        ...
        f.write((char*)&kf->mvDepth[i], sizeof(float));           // float
